...
public class FruitNinja extends ApplicationAdapter implements InputProcessor {
    ...
    public void create() {
        ...
        //ekran boyutuna göre radiusu ayarlıyoruz.
        Fruit.radius = Math.max(Gdx.graphics.getHeight(), Gdx.graphics.getWidth()) / 20f;

        //kullanıcının girdisini işleme sokacak aktivite
        Gdx.input.setInputProcessor(this);
        //font generator sınıfı ile daha düzgün fontlarda çalışmak
        ...
        parameter.size = 40;
        parameter.characters = "0123456789 CutTopPlaySscre:.+-";
    ...
    public void render() {
        ...
        //meyveleri ekrana gönderme sıra ve süremizi takip etmek için zamanlar düzenleme
        ...
        if (lives > 0) {
            ...
            for (int i = 0; i < lives; i++) {
                batch.draw(apple, i * 40f + 50f, Gdx.graphics.getHeight() - 35f, 35f, 35f);
            }
            ...
            //elmalar aşağı düşünce hakkın azalması
    ...
    private void addItem() {
        ...
        //zamanın yüzde ikisinde yani çok düşük bir zamanda
        if (type > 0.98) {
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        if (lives <= 0 && currentTime - gameOverTime > 2f) {
            ...
            //game mode
            //kullanıcı eğer meyvenin üzerine tıkladıysa çıkartılacak
            Array<Fruit> toRemove = new Array<Fruit>();
            Vector2 pos = new Vector2(screenX, Gdx.graphics.getHeight() - screenY);
            int plusScore = 0;
            for (Fruit fruit : fruitArray) {

                System.out.println("Distance : " + pos.dst(fruit.pos));
                System.out.println("Distance clicked: " + fruit.clicked(fruit.pos));
                System.out.println("Distance radius: " + Fruit.radius * Fruit.radius + 1);
